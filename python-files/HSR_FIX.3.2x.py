...
VALID_HASH_TRIOS: dict[str, tuple[str, str, str]] = {
    ...
    "SundayHair": ("947f1a00", "4342e04e", "d6ded331"),
    "SundayHead": ("bc1e3f68", "f5137a3d", "724c9e96"),
    "SundayBody": ("05ac0797", "8bb5eac2", "384256a2"),
    ...
    "AcheronHairBack": ("fe3d9074", "7ef00088", "36c43580"),
    ...
    "CaelusRemembranceHead": ("8d6ae530", "13e27600", "9e47ee7c"),
...
# ────────────────────────────────────────────────────────────────────
# ──────────────────────────────────────────────────────────────
# INI 후처리 로직 – 이 부분을 "# MARK: RUN" 바로 위에 붙여넣으세요

def _ini_comment_and_relocate(dir_path, pattern="*.ini", debug=False):
    import os, re, fnmatch
    # 1) override_vertex_count, override_byte_stride 전역 주석 처리
    pat0 = re.compile(r"^(\s*)(override_vertex_count|override_byte_stride)\s*=.*$")
    # 2) TextureOverride 내부 if/$\ 블록 재배치
    pat_section = re.compile(r"\[([^\]]+)\]")
    pat_if = re.compile(r"\s*if\s+\$\\")
    pat_match = re.compile(r"\s*match_priority\s*=")
    pat_endif = re.compile(r"\s*endif")
    pat_hash = re.compile(r"\s*hash\s*=")
    ...
    def comment_lines(lines):
        ...
        for L in lines:
            ...
            if m and not L.lstrip().startswith(";"):
                indent = m.group(1)
                out.append(indent + ";" + L[len(indent):])
            else:
                out.append(L)
        return out
    ...
    def relocate(lines):
        ...
        while i < len(lines):
            L = lines[i]
            m = pat_section.match(L)
            if m:
                sec = m.group(1)
                skip = (sec.strip().lower() == 'present')  # [Present] 섹션 스킵
                if not skip:
                    blocks.setdefault(sec, [])
                out.append(L)
                i += 1
                continue
            if sec and not skip and pat_if.match(L):
                blk = []
                if out and pat_match.match(out[-1]):
                    blk.append(out.pop())
                while i < len(lines):
                    blk.append(lines[i])
                    i += 1
                    if pat_endif.match(blk[-1]):
                        break
                blocks[sec].append(blk)
                continue
            out.append(L)
            i += 1

        res = []
        pending = {s: [b[-1] for b in bs] for s, bs in blocks.items()}
        curr = None
        for idx, L in enumerate(out):
            ...
            if m:
                curr = m.group(1)
            if curr and curr.strip().lower() != 'present' and pat_hash.match(L):
                for blk in blocks.get(curr, []):
                    for sub in blk[:-1]:
                        res.append(sub)
            nxt = out[idx + 1] if idx + 1 < len(out) else None
            if curr and curr.strip().lower() != 'present' and (nxt is None or pat_section.match(nxt)):
                for e in pending.get(curr, []):
                    res.append(e)
                curr = None
        return res
    ...
    for root,_,fs in os.walk(dir_path):
        for fn in fnmatch.filter(fs, pattern):
            if "DISABLED" in fn.upper(): continue
            ...
            try:
                ...
                continue
            data = comment_lines(data)
            data2 = relocate(data)
            if data2 != data:
                with open(full, "w", encoding="utf-8") as f:
                    f.writelines(data2)
                print(f"[INI] {fn} updated")
...
if __name__ == '__main__':
    try: main()
        ...
        print("\n[INI 후처리 시작]")
        _ini_comment_and_relocate(dir_path=os.getcwd(), pattern="*.ini", debug=False)
        print("[INI 후처리 완료]\n")
        input('\nPress "Enter" to quit...\n')
