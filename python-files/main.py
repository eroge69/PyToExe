...
class triggerbot:
    ...
    def cooldown(self):
        time.sleep(random.uniform(0.1, 0.3))  # Retraso aleatorio para hacer el tiempo de reacción más humano
        with self.toggle_lock:
            self.triggerbot_toggle = True
            kernel32.Beep(440, 75) if not self.triggerbot else kernel32.Beep(700, 100)
    ...
    def searcherino(self):
        ...
        if self.triggerbot and len(matching_pixels) > 0:
            ...
            time.sleep(random.uniform(actual_delay - 0.05, actual_delay + 0.05))  # Variación en el tiempo de reacción
    ...
    def hold(self):
        while True:
            while win32api.GetAsyncKeyState(self.trigger_hotkey) < 0:
                ...
                time.sleep(random.uniform(0.05, 0.15))  # Retraso aleatorio para el comportamiento humano
