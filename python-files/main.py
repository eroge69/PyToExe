...
def choices():
    ...
    while scope not in choise:
        try:
            scope = int(input('Выберите диапозон:'))
        except:
...
def cmdCommand(command):
    content = subprocess.run([command], capture_output=True, text=True, encoding="cp866")
    ...
    while i <254:
        ...
        if len(match)!=0:
            ...
            if i < ip:
                ...
                print("Если вы читаете это сообщение, значит произошла ошибка при анализе свободных ip адресов. Это сообщение будет появляться не больше 254 раз")
        else:
            i+=1
    print('Свободные ip-адреса:', '  '.join(validIP))
