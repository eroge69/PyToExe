...
class FileEncryptorApp:
    def __init__(self, master):
        self.master = master
        master.title("SENC (Dateien Verschlüsseler)")

        self.filepath = ""
        Button(master, text="ℹ️ Über SENC", command=self.about).pack()
        ...
        self.encrypt_btn = Button(master, text="🔒 Verschlüsseln (Original -> SENC)", command=self.encrypt_action)
        self.encrypt_btn.pack(pady=5)

        self.key_entry = Entry(master, width=30, justify='center')
        self.key_entry.pack(pady=5)
        self.key_entry.insert(0, "SKEY (####-####-####-####)")

        self.decrypt_btn = Button(master, text="🔓 Entschlüsseln (SENC -> Original)", command=self.decrypt_action)
    ...
    def encrypt_action(self):
        ...
        self.status.config(text=f"Datei verschlüsselt (ORIGINAL->SENC):\n{os.path.basename(out_file)}\nKey: {key}", fg="green")
        self.key_entry.delete(0, END)
        self.key_entry.insert(0, key)
    def about(self):
        messagebox.showinfo("Über SENC", "SENC ist ein Programm, das mit einem 16-stelligen AES-256 Schlüssel via SHA-256 Dateien ver- oder entschlüsselt.")
    def decrypt_action(self):
        ...
        if result:
            self.status.config(text=f"Datei entschlüsselt (SENC->ORIGINAL):\n{os.path.basename(result)}", fg="green")
        else:
            self.status.config(text="Entschlüsselung fehlgeschlagen! Falscher Schlüssel / Keine Datei im SENC Format", fg="red")
