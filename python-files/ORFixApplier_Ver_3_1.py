...
def search_part(
    ...
    Returns the character name, part and a boolean indicating if it needs orfix"""
    for data in chara_name_position_ib_hash:
        ...
        for part, first_index in normal_parts:
            ...
            if ib_pattern.search(section) and match_pattern.search(section):
                return character, component + part, True
...
def apply_orfix(
    ...
    for i, section in enumerate(sections):
        ...
        chara_name, part, needs_orfix = search_part(
        ...
        if not needs_orfix:
            ...
            if merged_pattern.search(section):
                ...
                if ORFIX_PATTERN.search(commandlist):
                    ...
                    to_print.append(
                        f"\tORFIX removed for {chara_name}'s CommandList. It was not needed"
                    )
            ...
            if ORFIX_PATTERN.search(section):
                ...
                to_print.append(f"\tORFIX removed for {chara_name}. It was not needed")
            continue
