...
def xml_to_dataframe(xml_file_path):
    ...
    root = ET.fromstring("<어휘>" + xml_string + "</어휘>")
    rows = []
...
def dataframe_to_xml(df):
    # DataFrame을 XML 문자열로 변환
    root = ET.Element("어휘")
    for _, row in df.iterrows():
        항목 = ET.SubElement(root, "어휘항목", {"식별자": row[("기본", "식별자")]})
        for tag in ["문헌명", "항목", "음가", "속성", "어의", "어의보충"]:
            el = ET.SubElement(항목, tag)
            el.text = row[("기본", tag)] if pd.notna(row[("기본", tag)]) else ""
        
        # 번역용례 처리
        번역용례 = ET.SubElement(항목, "번역용례")
        for i in range(1, len([col for col in df.columns if col[0].startswith("번역용례")]) // 4 + 1):
            if pd.notna(row[(f"번역용례{i}", "용례 식별자")]):
                예 = ET.SubElement(번역용례, "용례", {"식별자": row[(f"번역용례{i}", "용례 식별자")]})
                for subtag in ["출전정보", "원문", "번역문"]:
                    subel = ET.SubElement(예, subtag)
                    subel.text = row[(f"번역용례{i}", subtag)] if pd.notna(row[(f"번역용례{i}", subtag)]) else ""
        
        # 참조사전 처리
        참조사전 = ET.SubElement(항목, "참조사전")
        for 사전 in row[("참조사전", "사전")].split("; "):
            if 사전.strip():
                사전_el = ET.SubElement(참조사전, "사전")
                사전_el.text = 사전.strip()
...
xml_data = r"C:/Users/kaien/Downloads/REPOSITORY/private725/LX2024/tools/어휘속성작업물(논어주소1).xml"
