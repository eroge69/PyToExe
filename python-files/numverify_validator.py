...
def splash_screen():
    f = Figlet(font='slant')
    print(Fore.MAGENTA + f.renderText('NUMVERIFY VALIDATOR'))
    print(Fore.CYAN + "=" * 60)
...
def browse_file():
    Tk().withdraw()
    file_path = filedialog.askopenfilename(title="Select the phone leads file")
    return file_path
...
def main():
    ...
    input(f"\n{Fore.CYAN}📁 Press Enter to load phone leads file...")
    ...
    try:
        ...
        with open(summary_path, "w") as f:
            ...
            f.write(f"⚠️  Skipped (wrong carrier): {skipped_count}\n")

        print(f"\n{Fore.GREEN}✅ Validation complete! Check the 'results' folder.")
        print(f"{Fore.CYAN}📄 Summary saved to results/summary.txt")
