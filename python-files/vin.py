...
def lexical_analyzer(text, file_path):
    ...
    def clean_and_report_invalid_fragments(text):
        ...
        def is_valid_char(c):
            return c.isalnum() or c == '_' or c == 'а' or c == 'А'  # поддержка русской "а" если нужно
    ...
    def check_keywords_and_report_errors(cleaned_text):
        ...
        # Проверка на наличие '=' и что после него
        ...
        if has_equal:
            ...
            # Если нет '=', пытаемся найти число после предполагаемого '='
            for i, token in enumerate(tokens):
                if token.isdigit():
                    # Допустим, это значение — тогда ошибка только про отсутствие '='
                    errors.append("нет токена (=)")
                    value_token = token  # не проверяем его дополнительно
                    break
        ...
        for token in tokens:
            ...
            # Пропускаем проверку value_token, если уже учли её выше
        ...
        # Проверяем только недостающие символы, кроме '=' если он уже обработан
        for symbol in required_symbols:
            if symbol == '=' and not has_equal:
                continue  # уже добавили ошибку про '=' выше
            if symbol not in cleaned_text:
                errors.append(f"нет токена ({symbol})")

        # Проверка корректности value_token, если оно есть и не состоит из цифр
        if value_token and any(char.isalpha() for char in value_token):
            errors.append(f"неверный фрагмент в числе ({value_token})")

        return errors


    # --- Вычисление расстояния Левенштейна для сравнения ключевых слов ---
    ...
    # Вывод отчёта об удалённых фрагментах
