...
def drawClonedRegions(image, matchedClusters, segments):
    for matchedClusterNumber in matchedClusters:
        ...
        rows1, cols1 = np.where(segments == matchedClusterNumber)
        ...
        if len(matchedClusters[matchedClusterNumber]) >= 1:
            for similarClusterNumber in matchedClusters[matchedClusterNumber]:
                ...
                for row2, col2 in zip(rows2, cols2):
                    image[row2, col2, 2] = 0  # making blue channel zero
        print('\n')
    
    # ↓↓↓ Resize output image here ↓↓↓
    resized_image = cv2.resize(image, None, fx=0.5, fy=0.5)  # Adjust fx and fy as needed
    cv2.imshow('Cloning Detection Result', resized_image)
    cv2.waitKey(0)
...
#Main
img = imageResize(cv2.imread(r"C:/Users/anshu/OneDrive/Desktop/WhatsApp Image 2025-04-15 at 13.28.45_770a043d.jpg"))
