...
class LogViewerApp:
        ...
        for i, line in enumerate(lines):
            if '<art>' in line:
                ...
                if '<tag>' in line:
                    ...
                    try:
                        order_line = next((l for l in lines[i:] if '<order>' in l), None)
                        cartId_line = next((l for l in lines[i:] if '<cartId>' in l), None)
                        order_val = int(order_line.split('<order>')[1].split('</order>')[0].split('-')[1])
                        cartId_val = int(cartId_line.split('<cartId>')[1].split('</cartId>')[0].split('-')[1])
                        if abs(cartId_val - order_val) in [1, 2]:
                            transformed_log.append(f"{line}  // Бронь в один клик!!!")
                        else:
                    ...
                    try:
                        if ',' in qty_str:
                            qty = float(qty_str.replace(',', '.'))  # Handle comma as decimal separator
                        else:
                            qty = int(qty_str)

                        global price_line
                        global base_price_line
                    ...
                    val_str = line.split('<val>')[1].split('</val>')[0]

                    try:
                        val = float(val_str)


                        if price_line and base_price_line:
                            price_str = price_line.split('<price>')[1].split('</price>')[0]
                            base_price_str = base_price_line.split('<basePrice>')[1].split('</basePrice>')[0]

                            base_price = float(base_price_str)
                            discount_percent = (val * 100) / base_price
                            if val != 0:
                                transformed_log.append(f"{line} // Скидка: {discount_percent:.2f}%")
                        else:
                            transformed_log.append(line)
                    except (ValueError, IndexError):
                        transformed_log.append(line)
    ...
    def search_log(self):
        ...
        lines = self.transformed_log_content.split('\n') if self.use_excel_data else self.raw_log_content.split('\n')
