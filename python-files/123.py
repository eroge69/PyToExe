def calculate_nds(amount):
    # Расчет НДС 20% (делим на 6)
    nds_20 = amount / 6
    amount_after_nds_20 = amount - nds_20
    
    # Расчет НДС 10% (делим на 11)
    nds_10 = amount / 11
...
def main():
    ...
    print("Расчет НДС 20% и 10% от введенной суммы.")
    ...
    try:
        ...
        print(f"Сумма после вычитания НДС: {amount_after_nds_20:.4f}")
        
        # Вывод результатов для НДС 10%
        print(f"\nРезультаты для ставки НДС 10%:")
        print(f"Сумма НДС: {nds_10:.4f}")
        print(f"Сумма после вычитания НДС: {amount_after_nds_10:.4f}")
